...
| Route | What it shows |
|---|---|
| `/` | Top-3 podium (Follow-on-X CTAs), equity-curve chart (every dot is one matured call — click it for the tweet, the token, and how much that call moved the line), token-charts grid ("who caught the winners"), full leaderboard table. Cohort + sort + **view** drive everything. The view is a three-way concentration split (Path A): **Scouts** (default) — handles whose single most-mentioned token is under 50% of their matured calls, i.e. diversified callers; **Insiders** — handles ≥50% concentrated on one token, overwhelmingly project accounts ranked on their own coin; **All** — unfiltered. Scores are always the honest full-record aggregate — the view only partitions handles, it does not re-score them. `?view=insiders` / `?view=all` switch tabs. |
